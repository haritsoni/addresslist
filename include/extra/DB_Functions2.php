...
class DB_Functions {
	...
	$resstatus=mysql_fetch_array($status);
	...
	$rescontact=mysql_fetch_array($contact);
    	...
    	$insqry="INSERT INTO `journal_main` (`period`,`journal_id`,`post_date`,`store_id`,`description`,`discount`,`terms`,`sales_tax`,`total_amount`,`currencies_code`,`currencies_value`,`so_po_ref_id`,`purchase_invoice_id`,`admin_id`,`gl_acct_id`,`bill_acct_id`,`bill_address_id`,`bill_primary_name`,`bill_address1`,`bill_address2`,`bill_city_town`,`bill_state_province`,
`bill_postal_code`,`bill_country_code`,`bill_telephone1`,`bill_email`,`ship_acct_id`,`ship_address_id`,`ship_primary_name`,`ship_contact`,`ship_address1`,`ship_address2`,`ship_city_town`,`ship_state_province`,`ship_postal_code`,`ship_country_code`,`ship_telephone1`,`ship_email`,`terminal_date`)values ('".date("n")."','10','".$date."','0','Sales Draft','".$discount."','".$rescontact['special_terms']."','".$gst."','".$amount."','SGD','1','0','".$resstatus['next_so_num']."','".$userId."','".$resconfig['configuration_value']."','".$customerId."','".$addressId."','".$company."','".$address1."','".$address2."','".$city."','".$state."','".$zipCode."','".$country."','".$TelephoneNo."','".$email."','".$customerId."','".$addressId."','".$company."','".$address3."','".$address4."','".$city."','".$state."','".$zipCode."','".$country."','".$TelephoneNo."','".$email."','".$date."')";
    	$result = mysql_query($insqry);
    	$ins_id = mysql_insert_id();

		if(isset($productName)){
			foreach($$productName as $key=>$product){	
				$ins1=mysql_query("INSERT INTO `journal_item`(`ref_id`, `item_cnt`, `gl_type`, `sku`, `qty`, `description`, `debit_amount`, `credit_amount`, `gl_account`, `full_price`, `post_date`) VALUES ('".$ins_id."','1','sos','".$productName[$key]."','".$quantity[$key]."','".$productName[$key]."','debit_amt','credit_amt','".$resconfig['configuration_value']."','".$subtotal."','".$date."')") or die(mysql_error());

				$ins2=mysql_query("INSERT INTO `journal_item`(`ref_id`, `item_cnt`, `gl_type`, `qty`, `description`, `debit_amount`, `credit_amount`, `gl_account`, `full_price`, `post_date`) VALUES ('".$ins_id."','0','tax','1','GST','debit_amt','credit_amt','".$resconfig['configuration_value']."','0','".$date."')") or die(mysql_error());

				$ins3=mysql_query("INSERT INTO `journal_item`(`ref_id`, `item_cnt`, `gl_type`, `qty`, `description`, `debit_amount`, `credit_amount`, `gl_account`, `full_price`, `post_date`) VALUES ('".$ins_id."','0','dsc','1','Sales Draft-Discount','debit_amt','credit_amt','".$resconfig['configuration_value']."','0','".$date."')") or die(mysql_error());

				$ins4=mysql_query("INSERT INTO `journal_item`(`ref_id`, `item_cnt`, `gl_type`, `qty`, `description`, `debit_amount`, `credit_amount`, `gl_account`, `full_price`, `post_date`) VALUES ('".$ins_id."','0','ttl','1','Sales Draft-Total','debit_amt','credit_amt','".$resconfig['configuration_value']."','0','".$date."')") or die(mysql_error());
    ...
    public function getOrderDetails($customerId,$invoiceId) {
    	//journal id 10 is used for order and 12 is used for invoice
			...
			if($customerId!="")
			 {
				$whr .= "  AND `bill_address_id` = '$customerId'";
			 }
			 if($invoiceId!="")
			 {
